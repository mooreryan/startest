...
pub fn run_tests(test_files: List(TestFile), ctx: Context) {
  ...
  let executed_tests =
    list.map(test_cases, run_test_task) |> list.map(task.await_forever)

  let executed_tests =
    list.map(list.zip(test_cases, executed_tests), fn(tup) {
      ExecutedTest(test_case: tup.0, outcome: tup.1)
    })
...
fn run_test_task(test_case: Test) -> task.Task(TestOutcome) {
  task.async(fn() { run_test(test_case) })
}
